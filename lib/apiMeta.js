...
const API_META = {
  ...
  endpoints: {
    ...
    '/api/ci': 'Pass or fail JSON. HTTP 200 after warmup even if ok=false; 503 only while warming. Use spark_ok for Spark-only gates.',
    ...
    '/api/alerts': 'Alert channel status (Telegram / webhook) and recent fleet events (no secrets).',
    '/api/check': 'POST operator self-check: light probe of host:port (SSRF-safe, rate-limited).',
  },
  probes: [
